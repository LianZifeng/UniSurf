...
from utils import topo_correct, laplacian_smooth
...
class LoadSurface(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SDF = processSDF(SDF)
        if self.surf_hemi == 'left':
            v_ras, f = nib.freesurfer.read_geometry(os.path.join(filepath, "lh.white"))
        elif self.surf_hemi == 'right':
            v_ras, f = nib.freesurfer.read_geometry(os.path.join(filepath, "rh.white"))
        f = f.astype(np.int32)
    ...
    for epoch in range(config.epochs):
        ...
        for idx, data in enumerate(train_loader):
            ...
            if epoch < config.warmup:
                ...
                # Compute gradient with respect to SDF
                optimizer.zero_grad()
